...
#define ICMP_BUFIZ IP_PAYLOAD_SIZE_MAX
...
icmp_input(const uint8_t *data, size_t len, ip_addr_t src, ip_addr_t dst, struct ip_iface *iface)
{
    ...
    case ICMP_TYPE_ECHO:
        ...
        if (icmp_output(ICMP_TYPE_ECHOREPLY, hdr->code, hdr->values, data + ICMP_HDR_SIZE, len - ICMP_HDR_SIZE, iface->unicast, src) == -1) {
            errorf("echo back failed");
        }
        break;
    default:
        /* ignore */

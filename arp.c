...
arp_cache_insert(ip_addr_t pa, const uint8_t *ha)
{
    ...
    debugf("INSERT: pa=%s (%x), ha=%s", ip_addr_ntop(cache->pa, addr1, sizeof(addr1)), pa, ether_addr_ntop(cache->ha, addr2, sizeof(addr2)));
    return cache;
}
...
arp_request(struct net_iface *iface, ip_addr_t tpa)
{
    ...
    memcpy(request.tha, ETHER_ADDR_ANY, sizeof(request.tha));
    memcpy(request.tpa, &tpa, sizeof(request.tpa));
    debugf("dev=%s, len=%zu", iface->dev->name, sizeof(request));
    arp_dump((uint8_t *)&request, sizeof(request));
    return net_device_output(iface->dev, NET_PROTOCOL_TYPE_ARP, (uint8_t *)&request, sizeof(request), ETHER_ADDR_BROADCAST);
}

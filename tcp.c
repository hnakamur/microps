...
#define TCP_FLG_ACK 0x10 /* Acknowledgment field significant */
...
#define TCP_DEFAULT_RTO 200000 /* micro seconds = 200ms */
...
tcp_retransmit_queue_cleanup(struct tcp_pcb *pcb)
{
    ...
    while (1) {
        ...
        debugf("remove, seq=%u, flags=%s, len=%u", entry->seq, tcp_flag_ntoa(entry->flg), entry->len);
        memory_free(entry);
    }
}
...
tcp_segment_arrives(struct tcp_segment_info *seg, uint8_t flags, uint8_t *data, size_t len, struct ip_endpoint *local, struct ip_endpoint *foreign)
{
    ...
    if (!pcb || pcb->state == TCP_PCB_STATE_CLOSED) {
        ...
        /* Send back ACK when a segment arrives at a unused port */
    ...
    case TCP_PCB_STATE_SYN_SENT:
        ...
        if (TCP_FLG_ISSET(flags, TCP_FLG_SYN)) {
            ...
            if (pcb->snd.una > pcb->iss) {
                 ...
                 * queue them for processing after ESTABLISHED state has been reached
                 */
                return;
            }
        }
...
static void
tcp_input(const uint8_t *input, size_t input_len, ip_addr_t src, ip_addr_t dst, struct ip_iface *iface)
{
    struct tcp_hdr *hdr;
    struct pseudo_hdr pseudo;
    uint16_t psum;
    ...
    if (src == IP_ADDR_BROADCAST || dst == IP_ADDR_BROADCAST) {
        errorf("neither src nor dst must not be broadcast, src=%s, dst=%s",
    ...
    mutex_lock(&mutex);
    tcp_segment_arrives(&seg, hdr->flg, (uint8_t *)hdr + hlen, input_len - hlen, &local, &foreign);
    mutex_unlock(&mutex);
}
...
tcp_open_rfc793(struct ip_endpoint *local, struct ip_endpoint *foreign, int active)
{
    ...
    if (active) {
        debugf("active open: local=%s, connecting...", ip_endpoint_ntop(local, ep1, sizeof(ep1)));
        pcb->local = *local;
        pcb->foreign = *foreign;
        pcb->rcv.wnd = sizeof(pcb->buf);
        pcb->iss = random();

...
ip_dump(const uint8_t *data, size_t len)
{
    ...
    hl = hdr->vhl & 0x0f;
    hlen = hl << 2;
    fprintf(stderr, "        vhl: 0x%02x [v: %u, hi: %u (%u)]\n", hdr->vhl, v, hl, hlen);
...
ip_input(const uint8_t *data, size_t len, struct net_device *dev)
{
    ...
    for (entry = protocols; entry; entry = entry->next) {
        if (entry->type == hdr->protocol) {
            entry->handler(data + hlen, len - hlen, hdr->src, hdr->dst, iface);
            return;
        }
    }
...
ip_output_core(struct ip_iface *iface, uint8_t protocol, const uint8_t *data, size_t len, ip_addr_t src, ip_addr_t dst, ip_addr_t nexthop, uint16_t id, uint16_t offset)
{
    ...
    debugf("dev=%s, dst=%s, nexthop=%s, protocol=%u, len=%u",
        NET_IFACE(iface)->dev->name,
        ip_addr_ntop(dst, addr1, sizeof(addr1)),
        ip_addr_ntop(nexthop, addr2, sizeof(addr2)),
        protocol, total);
    ip_dump(buf, total);
